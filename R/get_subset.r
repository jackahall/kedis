...
#' @param n_in inner loop (NULL for non-nested Cross-Validation)
#' @param ... additional parameters
#'
#' @return either a kd_cv_data or a kd_ncv_data, depending on the index and n_in
#' @export
get_subset <- function(data, idx, n_out, n_in = NULL, ...){
...
get_subset.kd_cv_index <- function(data, idx, n_out, ...){
  train <- kedis::prepare_data(data$raw$shapes[which(idx != n), ],
                               data$raw$covariates,
                               data$raw$population,
  ...
  test <- kedis::prepare_data(data$raw$shapes[which(idx == n), ],
                              data$raw$covariates,
                              data$raw$population,
                              data$names$filter_var,
                              data$names$response_var,
